...
function updateposteda_redirect(string $message): void
{
    echo '<script type="text/javascript">alert(' . json_encode($message) . ');window.location=\'updateposta.php\';</script>';
    exit;
}
...
if ($no === '' || $date === '' || $exdate === '' || $sdate === '' || $edate === '' || $infor === '') {
    updateposteda_redirect('All update fields are required.');
}

$stmt = mysqli_prepare($conn, "UPDATE postss SET dates = ?, Ex_date = ?, start_date = ?, end_date = ?, info = ? WHERE no = ?");
if (!$stmt) {
    updateposteda_redirect('Unable to prepare the update.');
}
...
if (!$saved) {
    updateposteda_redirect('Notice was not updated. ' . $errorMessage);
}

updateposteda_redirect('Successfully Updated.');
?>
