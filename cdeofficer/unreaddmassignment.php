...
<?php echo '<a href="sendall.php?id='.$r['type'].'">Send To Finance</a>';?>	
									<font size="3px">
<?php
$sql = "select * from payment_table where unread='yes' and status='no' and type='massignment'";
	 
	//Create a PS_Pagination object
	$pager = new PS_Pagination($conn, $sql, 10, 1);
 	//The paginate() function returns a mysql result set for the current page
	$rs = $pager->paginate();									
$query1 = mysql_query("select * from payment_table where unread='yes' and status='no' and type='massignment'")
or die(mysql_error());
...
<td><?php echo '<a rel="facebox" href="calculatemassignment.php?id='.$row['no'].'">Fill and calculate pay form</a>';?></td>
</div>
