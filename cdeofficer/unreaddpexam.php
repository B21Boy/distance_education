...
<?php echo '<a href="sendall.php?id='.$r['type'].'">Send To Finance</a>';?>								
									<font size="3px">
<?php
$sql = "select * from payment_table where unread='yes' and status='no' and type='pexam'";
	 
	//Create a PS_Pagination object
	$pager = new PS_Pagination($conn, $sql, 10, 1);
 	//The paginate() function returns a mysql result set for the current page
	$rs = $pager->paginate();									
$query1 = mysql_query("select * from payment_table where unread='yes' and status='no' and type='pexam'");
...
<td><?php echo '<a rel="facebox" href="calculatepfexam.php?id='.$row['no'].'">Fill and calculate pay form</a>';?></td>
</div>
